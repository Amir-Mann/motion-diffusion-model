...
def compute_into_camera_shift_from_angle(data, hor_angle):
    ...
    Returns: torch.Tensor: A data of shape [batch_size, 3], containing (max_x, avg_y, max_z) for each batch."""
    ...
    #Here calculate the max_indecies which give the largest transformed_values.
    max_indices = transformed_values.reshape(transformed_values.shape[0], -1).argmax(dim=-1)  # Shape: [batch_size]

    # Convert flat indices back to original data dimensions
    max_joint_indices = max_indices // transformed_values.shape[2]# Joint indices
    max_frame_indices = max_indices % transformed_values.shape[2]  # Frame indices

    # Gather the corresponding max_x and max_z values
    max_x_values = x[torch.arange(data.shape[0]), max_joint_indices, max_frame_indices]
    max_z_values = z[torch.arange(data.shape[0]), max_joint_indices, max_frame_indices]

    avg_y = y.mean(dim=(1, 2))  # Average over joints and frames
    return torch.stack([max_x_values, avg_y, max_z_values], dim=-1)  # Shape: [batch_size, 3]
...
def sample_random_camera(data, distance_factor=2.5):
    batch_size = data.shape[0]
    cam_hor_angles = torch.rand(batch_size, device=data.device) * 2 * np.pi  # [0, 2*pi]
    cam_ver_angles = (torch.rand(batch_size, device=data.device) - 0.75) * 2 * (np.pi / 12)  # [-pi/8, pi/24]
